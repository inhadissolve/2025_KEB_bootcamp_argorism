...
def find_vertex_bfs(g, find_vtx) -> bool:
    queue = deque()
    visited_ary = set()

    queue.append(0)  # 시작 노드 (춘천, 0번)
    visited_ary.add(0)
    ...
    while queue:
        ...
        if current == find_vtx:
            return True  # 찾고자 하는 정점이 존재하면 True

        for vertex in range(g_size):
            if g.graph[current][vertex] != 0 and vertex not in visited_ary:
                queue.append(vertex)
                visited_ary.add(vertex)

    return False  # 끝까지 탐색했지만 찾지 못하면 False
